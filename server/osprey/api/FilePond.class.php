...
function sanitize_filename_part($str) {
    return preg_replace("/[^a-z%A-Z0-9\_\s-]/", "", $str);
}
...
function write_file($path, $data, $filename) {
    error_log($path . DIRECTORY_SEPARATOR . $filename);
...
function move_file($file, $path) {
    global $logger;
    $logger->info('Moving file from :'.implode($file).': to :' .$path . ': ');
    
    if (is_uploaded_file($file['tmp_name'])) {
        $logger->info('Move temp file');
        return move_temp_file($file, $path);
    }
    $logger->info('Rename temp file');
...
function get_transfer($path, $id) {
    ...
    $logger->info('get_transfer a '.strval(preg_match('/^[0-9a-fA-F]{32}$/', $id)));
    $logger->info('get_transfer a :'.strval($id).':');
    
    if (!is_valid_transfer_id($id)) return false;

    $logger->info('get_transfer a');
    $transfer = new Transfer($id);

    $logger->info('get_transfer b');
    $path = $path . DIRECTORY_SEPARATOR . $id;

    $logger->info('get_transfer c');
    $file = get_file($path, '*.*');
    
    $logger->info('get_transfer d');
    $metadata = get_file($path, METADATA_FILENAME);
    $data = read_file_contents($metadata['tmp_name']);
    $logger->info('get_transfer e'.print_r($metadata,true));
    $variants = get_files($path . DIRECTORY_SEPARATOR . VARIANTS_DIR, '*.*');

    $logger->info('get_transfer f');
    $transfer->restore($file, $variants, null, @json_decode($data) ); //from metadata....

    $logger->info('get_transfer g');
    return $transfer;
}
...
function route_api_request($entries, $routes) {
    ...
    $logger->info("route_api_request " . $request_method);
    ...
    foreach ($entries as $entry) {
        ...
        if ($request_method === 'DELETE') {
            ...
            return call_user_func($routes['REVERT_FILE_TRANSFER'],trim($contents));
        }
