...
function handle_file_transfer($transfer) {
    global $logger;
    $logger->info("handler file transfer");
...
function handle_revert_file_transfer($id) {
    global $logger;
    $logger->info("handle_revert_file_transfer:" . FilePond\is_valid_transfer_id($id));
    $logger->info("handle_revert_file_transfer:" . $id . ":");
    // test if id was supplied
    if (!isset($id) || !FilePond\is_valid_transfer_id($id)) return http_response_code(400);
    $logger->info("handle_revert_file_transfer:: valid transfer id");
    // remove transfer directory
    FilePond\remove_transfer_directory(TRANSFER_DIR, $id);
    $logger->info("handle_revert_file_transfer:: removed dir    ");
