...
class Wp_Osprey_Upload {
	...
	public function uploadedImgs( $attrs =[] , $contnet = null, $tag = ''){
		...
		if ( is_user_logged_in() ) {
			...
			if( array_key_exists("admin",$attrs) && (current_user_can('editor') || current_user_can('administrator') )) {
				$form = $this->readAllDb();
				return '<div>'.$form.'</div>';
			} else {
...
function readDb($name){
	...
	$queryresult = $newdb->get_results( "SELECT * FROM Uploads WHERE username='".$name."'");
